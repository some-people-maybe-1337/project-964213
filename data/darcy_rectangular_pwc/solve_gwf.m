...
function P = solve_gwf(coef,F,~)
    ...
    solutionVector = A\F(:);  % Solve the linear system
    solutionMatrix = reshape(solutionVector, K-2, K-2);  % Reshape the solution vector to a matrix
    P = [zeros(1, K); [zeros(K-2, 1), solutionMatrix, zeros(K-2, 1)]; zeros(1, K)];
